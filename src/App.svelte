<script lang="ts">
	...
	let openedFormId = $state(null);
	let openedCursByForm = $state({});
	...
	function toggleCurs(formId, curs) {
		...
		const currentOpenedCurs = openedCursByForm[formId];
		
		if (currentOpenedCurs === curs) {
			openedCursByForm[formId] = null;
		} else {
			openedCursByForm[formId] = curs;
		}
		openedCursByForm = { ...openedCursByForm };
	}
