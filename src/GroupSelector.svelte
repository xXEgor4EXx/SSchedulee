<script>
	...
	// Вместо expandedForms и expandedCurs используем новую логику
	export let openedFormId = null; // ID открытой формы
	export let openedCursByForm = {}; // Открытые курсы по формам
	...
	{#each formEduData as form}
		<div class="tree-node">
			<div class="folder" on:click={() => toggleForm(form.FormEdu_ID)}>
				<span class="arrow">{openedFormId === form.FormEdu_ID ? '▼' : '▶'}</span>
				<span class="folder-name">{form.FormEduName}</span>
			</div>
			
			{#if openedFormId === form.FormEdu_ID}
				<div class="nested">
					{#each form.arr as cursItem}
						<div class="tree-node">
							<div class="folder" on:click={() => toggleCurs(form.FormEdu_ID, cursItem.Curs)}>
								<span class="arrow">{openedCursByForm[form.FormEdu_ID] === cursItem.Curs ? '▼' : '▶'}</span>
								<span class="folder-name">Курс {cursItem.Curs}</span>
							</div>
							
							{#if openedCursByForm[form.FormEdu_ID] === cursItem.Curs}
								<div class="nested">
